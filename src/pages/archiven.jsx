...
export default function Archives() {
  ...
  const [upType, setUpType] = useState("Shahaado"); // "Shahaado" | "Sawir"
  ...
  const handleFile = (e) => {
    ...
    if (!file.type.startsWith("image/")) {
      setUpError("Fadlan sawir kaliya soo geli (JPG, PNG, WEBP).");
      return;
    }
    if (file.size > 5 * 1024 * 1024) {
      setUpError("Faylku waa inuu ka yar yahay 5MB.");
      return;
    }
    setUpError("");
    setUpFile(file);
  ...
  // Convert file to base64 and store in Firestore (no Storage needed)
  const handleUpload = async () => {
    ...
    try {
      ...
      setUpError("Khalad: " + err.message);
    }
    setUploading(false);
  };

  const shahaadoList = archives.filter((a) => a.type === "Shahaado" && a.approved);
  const sawirList = archives.filter((a) => a.type === "Sawir" && a.approved);
  ...
  return (
    <div className="min-h-screen text-white" style={{ background: "#0d0d0d" }}>
      ...
      {/* HERO */}
      <div
        className="text-center px-5 py-14 md:py-20"
        style={{
      ...
      <div className="flex justify-center gap-3 px-5 py-6">
        {[
          ...
          { key: "upload", label: "⬆️ Soo Geli" },
        ].map((t) => (
          <button
            key={t.key}
            onClick={() => { setTab(t.key); setUploadDone(false); }}
            style={{
              padding: "10px 24px",
              borderRadius: "12px",
              fontWeight: 700,
      ...
      {tab === "upload" && (
        <div className="px-5 pb-20 flex justify-center">
          <div
            className="w-full max-w-lg rounded-3xl p-7 md:p-10"
            ...
            {uploadDone ? (
              <div className="text-center py-6">
                ...
                <h3 className="text-2xl font-black mb-2">
                  La <span style={{ color: "#f5c518" }}>Gudbiyay!</span>
                </h3>
                <p className="text-sm mb-6" style={{ color: "#64748b" }}>
                  Codsigaagu waa la helay. Admin-ku wuu eegi doonaa oo wuu dhejin doonaa gallery-ga.
                </p>
                <button
              ...
              <>
                <h2 className="text-2xl font-black mb-6" style={{ color: "#f5c518" }}>
                  ⬆️ Soo Geli Sawirgaaga
                </h2>
                ...
                <textarea
                  ...
                  placeholder="Sharax yar ama faallo..."
                  rows={3}
                ...
                >
                  {upPreview ? (
                    <img
                      src={upPreview}
                      alt="preview"
                      style={{ maxHeight: 220, borderRadius: 10, objectFit: "contain", margin: "0 auto", display: "block" }}
                    />
                  ) : (
                    <>
                ...
                <p style={{ textAlign: "center", color: "#2d3748", fontSize: 11, marginTop: 12 }}>
                  Admin-ku wuu eegi doonaa kahor inta aan la dhejin.
                </p>
              </>
            )}
          </div>
      ...
      {lightbox && (
        ...
        >
          <div
            style={{ maxWidth: 680, width: "100%", borderRadius: 20, overflow: "hidden", background: "#111", border: "1px solid rgba(245,197,24,0.2)" }}
