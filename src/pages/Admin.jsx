...
// ── Shared card style (top-level, used by new admin pages) ────────────────────
const CARD = {
...
// ═══════════════════════════════════════════════════════════════════════════════
// Firestore reading helpers (live data for the admin views)
// ═══════════════════════════════════════════════════════════════════════════════
...
function ErrorBox({ msg }) {
  return (
    <div style={{
      ...
      background: "#2a0a0a", border: `1px solid ${C.errorRed}40`, color: C.errorRed,
    }}>
...
// ── Generic document card (renders any Firestore doc) ─────────────────────────
  ...
  return (
    <div className="card-hover" style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        {sc && (
          <span style={{ background: sc.bg, color: sc.fg, padding: "4px 10px", borderRadius: 6, fontSize: 10, fontWeight: 700, flexShrink: 0 }}>
            {String(status)}
          </span>
        )}
      </div>
      ...
      {imgKey && (
        <img src={data[imgKey]} alt="" style={{ width: "100%", height: 130, objectFit: "cover", borderRadius: 8, border: `1px solid ${C.border}` }} />
      )}
        ...
        {rows.map(([k, v]) => {
          ...
          return (
            <div key={k} style={{ display: "flex", justifyContent: "space-between", gap: 10, padding: "6px 0", borderTop: `1px solid ${C.border}40` }}>
              <span style={{ color: C.textSub, fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".4px", flexShrink: 0, paddingTop: 1 }}>
                {prettyKey(k)}
              </span>
              {isLink
...
// ── Pure presenter for a collection ──────────────────────────────────────────
...
// ── Self-loading collection panel (used by the Psychology nav) ────────────────
...
// ── All Data hub (every collection in one place, with live counts) ────────────
function DataExplorer() {
  ...
  return (
    <div>
      {/* Live count chips for every collection */}
          ...
          return (
            <button
              ...
              style={{
                ...
                background: isActive ? C.redDim : C.surfaceCard,
                border: `1px solid ${isActive ? C.red : C.border}`,
                borderRadius: 14, padding: "16px 18px",
                boxShadow: isActive ? `0 6px 24px ${C.redGlow}` : "none",
              }}
            >
              <div style={{ color: isActive ? C.redLight : C.textMuted, fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 8 }}>
                {t.label}
              </div>
              <div style={{ color: C.text, fontSize: 28, fontWeight: 900, lineHeight: 1 }}>
                {s.loading ? "…" : s.docs.length}
              </div>
...
// ── Traders page (set the percentage shown to traders + view their psychology) ─
function EnrollmentsPage() {
  ...
  const q = search.trim().toLowerCase();
  const list = q
    ? enrollments.filter((e) => JSON.stringify(e).toLowerCase().includes(q))
    : enrollments;
  ...
  return (
    <div>
      <div style={{
        ...
        background: C.redFaint, border: `1px solid ${C.borderRed}`,
      }}>
        <span style={{ color: C.redLight }}><Icon.LockOpen /></span>
        <span style={{ color: C.textMuted, fontSize: 13 }}>
          Marka aad "Approve" gujiso, <strong style={{ color: C.redLight }}>enrollments/&#123;id&#125;.approved = true</strong>
          ayaa la dejinayaa — appka ardaygu wuxuu arki karaa course-ka oo furan.
        </span>
      </div>
      ...
      {loading ? <LoadingBox /> : list.length === 0 ? <EmptyBox name="enrollments" /> : (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {list.map((e) => {
            ...
            return (
              <div key={e.id} style={{
                ...CARD, padding: 16, display: "flex", alignItems: "center",
                ...
                <span style={{
                  background: approved ? C.greenDim : C.amberDim,
                  color: approved ? C.green : C.amber,
                ...
                <button
                  onClick={() => approve(e, !approved)}
                  disabled={busy[e.id]}
                  className="red-btn"
                  style={{
                    background: approved ? "transparent" : C.red,
                    color: approved ? C.errorRed : "#fff",
                    border: approved ? `1px solid ${C.errorDim}` : "none",
                    borderRadius: 8, padding: "9px 18px", fontSize: 13, fontWeight: 700,
                    cursor: busy[e.id] ? "not-allowed" : "pointer",
                    boxShadow: approved ? "none" : `0 4px 14px ${C.redGlow}`,
                  }}
                >
...
// ── Courses page (locked until paid) ──────────────────────────────────────────
              ...
              <div key={c.id} className="card-hover" style={{ ...CARD, padding: 0, overflow: "hidden", display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ position: "relative", height: 140, background: C.surface }}>
                  {c.thumbnailURL
                    ...
                    : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: C.textSub, fontSize: 36 }}>
                        {c.type === "Video" ? "🎬" : "📄"}
                      </div>}
                  {/* Lock badge */}
                  <div style={{
                    position: "absolute", top: 10, right: 10, display: "flex", alignItems: "center", gap: 6,
                    background: locked ? C.errorDim : C.greenDim, color: locked ? C.errorRed : C.green,
                  ...
                  <div style={{
                    ...
                    background: c.type === "PDF" ? C.errorDim : C.amberDim, color: c.type === "PDF" ? C.errorRed : C.amber,
                    padding: "5px 10px", borderRadius: 8, fontSize: 11, fontWeight: 700,
                  }}>{c.type || "—"}</div>
                </div>
                ...
                <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 10, flex: 1 }}>
                  ...
                  {c.description && (
                    <div style={{ color: C.textMuted, fontSize: 12, lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                      {c.description}
                    </div>
                  )}
                  <div style={{ marginTop: "auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                    <span style={{ color: price > 0 ? C.redLight : C.green, fontWeight: 900, fontSize: 20 }}>
                    ...
                    <div style={{ display: "flex", gap: 6 }}>
                      <button
                        ...
                        disabled={busy[c.id]}
                        title={locked ? "Unlock course" : "Lock course"}
                        style={{
                          display: "flex", alignItems: "center", gap: 5,
                          background: "transparent", border: `1px solid ${C.border}`, color: locked ? C.green : C.errorRed,
...
const RedBtn = ({ children, onClick, disabled, style = {} }) => (
  <button
    ...
    className="red-btn"
    style={{
      background: disabled ? "#1a1a1a" : C.red,
      ...
      alignItems: "center",
      gap: 8,
      boxShadow: disabled ? "none" : `0 4px 16px ${C.redGlow}`,
      ...style,
    }}
  >
    {children}
...
const StatCard = ({ icon, label, value, delta, redFill, sparkData }) => {
  ...
  const areaColor = redFill ? "rgba(255,80,80,0.2)" : C.redDim;
  ...
  return (
    <div
      ...
      style={{
        background: redFill
          ? `linear-gradient(135deg, #8b0000 0%, #cc1111 50%, #990000 100%)`
          : C.surfaceCard,
        border: redFill ? "none" : `1px solid ${C.border}`,
        borderRadius: 16,
        padding: "22px 24px",
        flex: 1,
        minWidth: 160,
        ...
        overflow: "hidden",
        boxShadow: redFill
          ? `0 8px 32px rgba(150,0,0,0.5), inset 0 1px 0 rgba(255,100,100,0.15)`
          : `0 4px 20px rgba(0,0,0,0.4)`,
      }}
    >
      {/* subtle red glow orb on normal cards */}
      {!redFill && (
        <div style={{
          position: "absolute", top: -30, right: -30, width: 120, height: 120,
          borderRadius: "50%", background: `radial-gradient(circle, ${C.redDim} 0%, transparent 70%)`,
          pointerEvents: "none",
        }} />
      )}
      ...
      <div style={{
        ...
        background: redFill ? "rgba(0,0,0,0.25)" : C.redDim,
        border: redFill ? "1px solid rgba(255,100,100,0.2)" : `1px solid ${C.borderRed}`,
        display: "flex", alignItems: "center", justifyContent: "center",
        color: redFill ? "#ffaaaa" : C.redLight,
      }}>
        {icon}
      </div>

      <div style={{ color: redFill ? "rgba(255,200,200,0.7)" : C.textMuted, fontSize: 11, fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", marginBottom: 4 }}>
        {label}
      </div>
      <div style={{ color: redFill ? "#fff" : C.text, fontSize: 32, fontWeight: 900, lineHeight: 1, marginBottom: 6 }}>
        {value}
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 5, color: redFill ? "#ffcccc" : C.redLight, fontSize: 12, marginBottom: 12 }}>
        <Icon.TrendUp /> <span>{delta}</span>
      </div>
      ...
      <svg viewBox={`0 0 ${w} ${h}`} style={{ width: "100%", height: 36 }}>
        <defs>
          <linearGradient id={`sg-${label}`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={lineColor} stopOpacity=".5" />
...
const IncomeSparkline = () => {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} style={{ width: "100%", height: 90, overflow: "visible" }}>
      <defs>
        <linearGradient id="income-sg" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor={C.red} stopOpacity=".45" />
          <stop offset="100%" stopColor={C.red} stopOpacity="0" />
        </linearGradient>
      </defs>
      <path d={area} fill="url(#income-sg)" />
      <path d={d} fill="none" stroke={C.redLight} strokeWidth={2.5} strokeLinejoin="round" strokeLinecap="round" />
      {xs.map((x, i) => i % 3 === 0 && (
        <circle key={i} cx={x} cy={ys[i]} r={3} fill={C.redLight} />
      ))}
    </svg>
  );
};
...
function Field({ label, value, onChange, placeholder, textarea, type = "text" }) {
  const base = {
    width: "100%", padding: "12px 16px", borderRadius: 10, marginBottom: 4,
    background: C.bgDeep, border: `1px solid ${C.border}`, color: C.text,
...
function UploadsTable({ uploads, onDelete, full }) {
  return (
    <div style={{
      background: C.surfaceCard, border: `1px solid ${C.border}`,
      borderRadius: 16, padding: 24, boxShadow: "0 4px 24px rgba(0,0,0,0.3)",
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        {!full && (
          <span style={{ fontSize: 12, color: C.textMuted, cursor: "pointer", padding: "4px 10px", borderRadius: 6, border: `1px solid ${C.border}` }}>
            View All →
          </span>
        )}
      </div>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          ...
          <tr>
            {["File Name", "Type", "Size", "Date", "Status", "Actions"].map((h) => (
              <th key={h} style={{
                ...
                color: C.textSub, fontWeight: 600, fontSize: 11,
        ...
        <tbody>
          {uploads.map((u) => (
            <tr key={u.id} className="row-hover" style={{ transition: "background .15s", borderRadius: 8 }}>
              <td style={{ padding: "13px 12px", borderBottom: `1px solid ${C.border}20` }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, color: C.text }}>
                  <div style={{
                    ...
                    background: u.type === "PDF" ? C.errorDim : C.amberDim,
                    color: u.type === "PDF" ? C.errorRed : C.amber,
                  }}>
              ...
              <td style={{ padding: "13px 12px", borderBottom: `1px solid ${C.border}20` }}>
                <div style={{ display: "flex", gap: 6 }}>
                  <button className="action-btn" style={{
                    ...
                    cursor: "pointer", padding: "6px 8px", borderRadius: 8, transition: "all .15s",
                  }}>
                    <Icon.Eye />
                  </button>
                  <button onClick={() => onDelete(u.id)} className="action-btn" style={{
                    background: "transparent", border: `1px solid ${C.errorDim}`, color: C.errorRed,
                    cursor: "pointer", padding: "6px 8px", borderRadius: 8, transition: "all .15s",
                  }}>
                    <Icon.Trash />
                  </button>
...
function PlaceholderPage({ label }) {
  return (
    <div style={{ textAlign: "center", paddingTop: 80 }}>
      ...
      <div style={{ color: C.redLight, fontWeight: 700, fontSize: 22, marginBottom: 8 }}>
...
export default function Admin() {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true); setUploadProgress(0); setUploadMsg("Uploading…");
    try {
      const mainFile = uploadTab === "pdf" ? pdfFile : videoFile;
      const folder = uploadTab === "pdf" ? "pdfs" : "videos";
      const mainRef = ref(storage, `${folder}/${Date.now()}_${mainFile.name}`);

      // 🚀 FAST UPLOAD — sawirka iyo file-ka isku mar (PARALLEL) ayay u baxayaan,
      // hadda lama sugayo in midba kan kale ka dambeeyo, markaa wuu degdegayaa.
      const mainPromise = uploadFileWithProgress(mainRef, mainFile, (p) => setUploadProgress(p));
      let thumbPromise = Promise.resolve("");
      if (thumbnail) {
        const tRef = ref(storage, `thumbnails/${Date.now()}_${thumbnail.name}`);
        thumbPromise = uploadFileWithProgress(tRef, thumbnail, () => {});
      }
      const [fileURL, thumbnailURL] = await Promise.all([mainPromise, thumbPromise]);

      setUploadMsg("Saving to database…"); setUploadProgress(100);
      const coursePrice = Number(price) || 0;
      const docData = {
        title, description, price: coursePrice, thumbnailURL,
        type: uploadTab === "pdf" ? "PDF" : "Video", fileURL,
        createdAt: Date.now(), status: "Published",
        // 🔒 Course-ku wuu xidhan yahay illaa ardaygu lacagta bixiyo
        // (price = 0 oo keliya ayaa bilaash oo furan ah).
        locked: coursePrice > 0,
        isPaid: false,
      };
      await addDoc(collection(db, "courses"), docData);
      setUploadProgress(100); setUploadMsg("✅ Uploaded successfully!");
      setUploads((prev) => [{
        id: Date.now().toString(),
        name: mainFile.name,
        type: uploadTab === "pdf" ? "PDF" : "Video",
        size: (mainFile.size / 1048576).toFixed(1) + " MB",
        date: new Date().toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }),
        status: "Published",
      }, ...prev]);
      setTitle(""); setDescription(""); setPrice("");
      setThumbnail(null); setPdfFile(null); setVideoFile(null);
    } catch (err) {
  ...
  const PAGE_TITLES = {
    ...
    dashboard: "Dashboard", upload: "Upload Content", uploads: "All Uploads",
    income: "Income", courses: "Courses", students: "Students",
    traders: "Traders", data: "All Data", psychology: "Psychology",
  ...
  const UploadZone = ({ tab, pdfFile, videoFile, setPdfFile, setVideoFile, inputId }) => {
    ...
    return (
      ...
      >
        <div style={{ color: file ? C.green : C.red, marginBottom: 10 }}>
          <Icon.UploadCloud />
        </div>
      ...
      {["pdf", "video"].map((t) => (
        <button
          ...
          style={{
            ...
            padding: compact ? "9px 0" : "10px 30px",
            borderRadius: 8, border: "none", cursor: "pointer",
            background: value === t ? C.red : "transparent",
            color: value === t ? "#fff" : C.textMuted,
            fontWeight: value === t ? 700 : 500,
            fontSize: 13,
            transition: "all .15s",
            display: "flex", alignItems: "center", justifyContent: "center", gap: 7,
            boxShadow: value === t ? `0 2px 10px ${C.redGlow}` : "none",
          }}
        >
          {t === "pdf" ? (
          ...
          <div style={{ padding: "20px 18px 16px", borderBottom: `1px solid ${C.border}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ color: C.red, fontWeight: 700, fontSize: 13, lineHeight: 1.3 }}>
                  TRADING
                </div>
                <div style={{ color: C.textSub, fontSize: 9, letterSpacing: 2, marginTop: 1 }}>
                  LEARN · TRADE · EARN
                </div>
              </div>
            </div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    transition: "all .15s",
                    boxShadow: active ? `0 4px 14px ${C.redGlow}` : "none",
                  }}
                >
                  <I />
                  {label}
          ...
          <div style={{ padding: "14px 16px", borderTop: `1px solid ${C.border}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>Admin</div>
              </div>
            </div>
            {/* Premium badge */}
            <div style={{
            ...
            <button
              style={{
                ...
                cursor: "pointer", fontSize: 14, transition: "color .15s",
              }}
          ...
          <header style={{
            ...
            <div>
              ...
              <div style={{ color: C.textMuted, fontSize: 13, marginTop: 1 }}>
                Welcome back, <span style={{ color: C.redLight }}>Admin</span>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <button style={{
                background: C.surfaceCard, border: `1px solid ${C.border}`,
                borderRadius: 10, padding: "9px 11px", color: C.textMuted,
                cursor: "pointer", position: "relative",
              }}>
                <Icon.Bell />
                <span style={{
                  position: "absolute", top: 7, right: 7, width: 8, height: 8,
                  background: C.red, borderRadius: "50%", border: `2px solid ${C.bgDeep}`,
                }} />
              </button>
              <div style={{
                display: "flex", alignItems: "center", gap: 10,
          ...
          <div style={{ padding: 28, flex: 1 }}>
            ...
            {activePage === "dashboard" && (
              <div>
                ...
                <div style={{ display: "flex", gap: 14, marginBottom: 24, flexWrap: "wrap" }}>
                  <StatCard icon={<Icon.Courses />} label="Total Courses" value={1} delta="15% this month"
                    sparkData={[20, 30, 25, 40, 35, 55, 50, 60, 58, 70]} />
                  <StatCard icon={<Icon.Upload />} label="Total Uploads" value={1} delta="12% this month"
                    sparkData={[40, 50, 45, 65, 55, 75, 70, 82, 78, 90]} />
                  <StatCard icon={<Icon.Students />} label="Total Students" value={1} delta="36% this month"
                    sparkData={[50, 55, 60, 58, 70, 72, 78, 82, 88, 95]} />
                  <StatCard icon={<Icon.Dollar />} label="Total Income" value="$0.00" delta="18% this month" redFill
                    sparkData={[30, 45, 38, 60, 52, 70, 65, 85, 78, 100]} />
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18, marginBottom: 24 }}>
                  ...
                  <div style={card}>
                    ...
                    <UploadZone
                      tab={uploadTab} pdfFile={pdfFile} videoFile={videoFile}
                      setPdfFile={setPdfFile} setVideoFile={setVideoFile}
                      inputId={`dash-file-${uploadTab}`}
                    />
                    <button
                      onClick={() => setActivePage("upload")}
                      style={{
                        marginTop: 12, width: "100%", padding: "10px 0", borderRadius: 10,
                        border: `1px solid ${C.borderRed}`, background: "transparent",
                        color: C.redLight, cursor: "pointer", fontSize: 13, fontWeight: 600,
                        transition: "all .15s",
                      }}
                  ...
                  <div style={card}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                      {[
                        { icon: "📄", title: "Upload PDF", sub: "Upload study materials, notes, guides & eBooks", tab: "pdf" },
                        ...
                        <div key={tab} style={{
                          ...
                          onMouseEnter={(e) => { e.currentTarget.style.borderColor = C.borderRed; e.currentTarget.style.boxShadow = `0 4px 20px ${C.redGlow}`; }}
                          onMouseLeave={(e) => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.boxShadow = "none"; }}
                        >
                          <div style={{ fontSize: 44, marginBottom: 10 }}>{icon}</div>
                          <div style={{ color: C.text, fontWeight: 700, marginBottom: 6, fontSize: 14 }}>{title}</div>
                          <div style={{ color: C.textMuted, fontSize: 12, marginBottom: 16, lineHeight: 1.5 }}>{sub}</div>
                          <RedBtn
                            onClick={() => { setUploadTab(tab); setActivePage("upload"); }}
                ...
                <div style={{ display: "grid", gridTemplateColumns: "3fr 2fr", gap: 18 }}>
                  ...
                  <div style={card}>
                    ...
                    <div style={{ color: C.textMuted, fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: ".5px", marginBottom: 4 }}>
                      Total Income
                    </div>
                    <div style={{ color: C.text, fontSize: 36, fontWeight: 900, marginBottom: 4 }}>$0.00</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 5, color: C.green, fontSize: 13, marginBottom: 2 }}>
                      <Icon.TrendUp /> <strong>+18%</strong>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginTop: 16 }}>
                      {[
                        ...
                        { label: "Withdrawn", val: "$2,350", delta: null },
                      ].map(({ label, val, delta }) => (
                        <div key={label} style={{
                          background: C.bgDeep, border: `1px solid ${C.border}`,
            ...
            {activePage === "upload" && (
              <div style={{ maxWidth: 720 }}>
                <TabSwitcher value={uploadTab} onChange={setUploadTab} />
                <div style={{ ...card, padding: 32 }}>
                  ...
                  <div style={{ marginBottom: 22 }}>
                    <label style={{ display: "block", color: C.textMuted, fontSize: 12, marginBottom: 8, fontWeight: 600, letterSpacing: ".4px", textTransform: "uppercase" }}>
                      Thumbnail (optional)
                    </label>
                    <input type="file" accept="image/*" onChange={(e) => setThumbnail(e.target.files[0])} style={{ color: C.textMuted, fontSize: 13 }} />
                  ...
                  <div style={{
                    ...
                    background: C.redFaint, border: `1px solid ${C.borderRed}`, color: C.textMuted,
                    display: "flex", alignItems: "center", gap: 8,
                  }}>
                    <span style={{ color: C.redLight }}><Icon.Lock /></span>
                  ...
                  {uploading && (
                    <div style={{ marginBottom: 16 }}>
                      ...
                      <div style={{ color: C.textMuted, fontSize: 12, marginTop: 5 }}>{uploadProgress}%</div>
                    </div>
                  )}
                  ...
                  {uploadMsg && (
                    <div style={{
                      ...
                      background: uploadMsg.startsWith("✅") ? "#0a2a18" : uploadMsg.startsWith("❌") ? "#2a0a0a" : "#1a1a0a",
                      border: `1px solid ${uploadMsg.startsWith("✅") ? C.green + "40" : uploadMsg.startsWith("❌") ? C.errorRed + "40" : C.amber + "40"}`,
                      color: uploadMsg.startsWith("✅") ? C.green : uploadMsg.startsWith("❌") ? C.errorRed : C.amber,
                    }}>
                      {uploadMsg}
                    </div>
                  )}

                  <RedBtn onClick={handleUpload} disabled={uploading} style={{ minWidth: 180 }}>
                    <Icon.Upload />
            ...
            {/* ════ COURSES (locked until paid) ════ */}
            {activePage === "courses" && <CoursesPage />}

            {/* ════ TRADERS (percentage + psychology) ════ */}
            {activePage === "traders" && <TradersPage />}

            {/* ════ ALL DATA (every collection in one place) ════ */}
